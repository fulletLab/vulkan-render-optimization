...
- The bottom Profiler tab reads `RendererStats` and exposes Vulkan frame, draw, triangle, culling, GPU resource residency, upload byte, CPU timing, light, and shadow counters without coupling engine runtime code to Qt.
- HDR/KTX2 environment assets, KTX2 supercompression transcoding, point/cascaded shadow coverage, complete shadow/material coverage, and fuller debug overlay coverage remain open renderer work; Qt must not become the primary 3D renderer.

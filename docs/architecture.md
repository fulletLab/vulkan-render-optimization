...
- The bottom Profiler tab reads `RendererStats` and exposes Vulkan frame, draw,
  ...
  pass-level CPU timing, light, shadow, and shadow-culling counters without coupling
  engine runtime code to Qt.
- HDR/KTX2 environment assets, compressed GPU target selection for KTX2 Basis/UASTC, point/cascaded shadow coverage, complete shadow/material coverage, and fuller debug overlay coverage remain open renderer work; Qt must not become the primary 3D renderer.
